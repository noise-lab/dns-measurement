...
\begin{tabular}{l|rr}
    ...
    \midrule
    SSL Connect Error & 5,374 & 26\% \\
    Couldn't Connect to Server & 16,690 & 8.2\% \\
    HTTP Error Status & 11,603 & 5.7\% \\
    Couldn't Decode Response & 8,099 & 4\% \\
    Couldn't Resolve the Resolver's Domain Name & 2,670 & 1.3\% \\
    SSL Certificate Error & 1,338 & 0.7\% \\
    Other Error & 75 & $<$ 1\% \\
    SSL Timeout & 1 & $<$ 1\% \\
    \midrule
    \textbf{Successful Responses} & 158,654 & 77.6\% \\
    \textbf{All Errors} & 45,850 & 22.4\% \\
    \bottomrule
\end{tabular}
\caption{Errors we received when trying to perform DoH queries from Ohio.}
\label{tab:errors}
\end{table}

We received responses from most resolvers that we queried.
\Fref{tab:unresponsive} shows the DoH resolvers that we failed to receive
a response from, and~\Fref{tab:errors} shows the most common errors we
received from attempting to communicate with the resolvers. The
