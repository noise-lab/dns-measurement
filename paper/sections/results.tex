...
We received responses from most resolvers that we queried.  \Fref{tab:errors}
...
\begin{figure*}
    ...
    \caption{Scatter plots of median ICMP ping latency vs. DoH query response times from each vantage point.}
    \label{fig:ping_qrt_scatters}
\end{figure*}

\Fref{fig:ping_qrt_scatters} compares median ICMP ping latencies to median 
...
server, and otherwise falls back to TLS 1.2, which requires an additional
network round trip. Thus, we can expect that ``normal'' DoH lookup times would
be on the order of 3--4 times the network latency.
We note that two resolvers--\texttt{doh.this.web.id} and 
\texttt{dns.twnic.tw}--are not shown on~\Fref{fig:Seoul_ping_qrt_scatter} 
...
From Frankfurt, we also observed two resolvers with median DoH query response 
