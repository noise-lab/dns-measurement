...
While the slowest resolver in Asia as measured from Asia was extremely slow,
the second-slowest resolver also exhibited slow performance, with a median
response time of about one second.
We observe more
...
\bottomrule
\end{tabular}
\caption{Median DNS response times for non-mainstream resolvers located in Asia.}
...
\bottomrule
\end{tabular}
\caption{Median DNS response times for non-mainstream resolvers located in Europe.}
\label{tab:UnconvEur}
\end{table}
...
\AH{Are we computing medians or averages for the ping data?
Make sure we use consistent units in the text}
